	...
	<c:choose>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				URL 1 : <a href="${link1}" target="_blank" title="새창열기">링크 1번</a><br/>
				URL 2 : <a href="${link2}" target="_black">링크 2번</a>
			</li>
		</c:when>
		...
		<c:when test="${step eq '10'}">
			<li>
				<h3>fn:indexOf() : 찾는 문자열이 내용에 있으면 문자열이 시작하는 index 값을 리턴 없으면 -1 리턴 </h3>
		...
		<li>
			<h3>fn:substring() : 주어진 시작 및 끝위치에 따라 문자열의 하위 집합을 반환</h3>
		...
		<li>
			...
			<%--중간의 공백은 HTML에서 보여줄때 한칸만 공백이 있는것처럼 보여줌. --%>
		</li>
		</c:when>
